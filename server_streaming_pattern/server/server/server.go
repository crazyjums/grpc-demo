...
func (s *Server) GetProduct(ctx context.Context, in *pb.ProductId) (*pb.Product, error) {
	log.Printf(common.LogPrefixServerStreaming+"GetProduct %s\n", in.Value)
	product, ok := s.productMap[in.Value]
	if !ok {
		return nil, errors.New(common.LogPrefixServerStreaming + "product is not exist! ")
	}
...
func (s *Server) AddProduct(ctx context.Context, in *pb.Product) (*pb.ProductId, error) {
	...
	if err != nil {
		log.Fatalf(common.LogPrefixServerStreaming+"failed to generate product id: %v", err)
	}
	...
	log.Printf(common.LogPrefixServerStreaming+"AddProduct %+v\n", product)
...
func (s *Server) SearchProducts(in *pb.ProductName, stream pb.ProductInfo_SearchProductsServer) error {
	...
	log.Printf(common.LogPrefixServerStreaming+"SearchProducts %s, curr productMap: %+v \n", in.Value, s.productMap)
	for _, product := range s.productMap {
		if strings.Contains(product.Name, in.Value) {
			err := stream.Send(product)
			if err != nil {
				log.Fatalf(common.LogPrefixServerStreaming+"failed to send: %v", err)
				return err
			}
			log.Printf(common.LogPrefixServerStreaming+"match product found key= %+v\n", in.Value)
